...
- 개요
    - 자바스크립트에서는 거의 모든 것이 객체다.
    - 함수 또한 객체다.
    - 이 말은 함수도 프로퍼티들을 가질 수 있다는 말이다.
    - 함수가 객체여서 오는 특징은 무엇이 있는지 알아보겠다.
        ```javascript
        function add(x, y) {
          return x + y;
        }
...
- 일급 객체란
    ...
    ```
    - 위에 나열한 기능이 모두 가능한 객체를 일급 객체라고 부른다.
    - 자바스크립트에서 함수는 이러한 특성을 다 지니고 있으며 이로써 함수형 프로그래밍이 가능하다.
    - 함수라고 특별한 무언가가 아니라 그냥 단순히 기능이 awesome한 **변수**라고 생각하면 편하다. (사실 그게 맞기도 하다.)
    
- 자바스크립트 함수의 특징
    - 함수의 인자로 전달
        - 함수(something)에 함수(me)를 인자로 전달 한 뒤 something 내에서 me를 호출할 수 있다. 
        - 이 사실을 처음 이해했을 때 너무 신기했다.
        - 너무도 많은 기능을 사용할 수 있었다.
    - 리턴 값으로 활용
        - 숫자가 리턴되는 경우나 문자열이 리턴되는 경우와 조금도 다를 게 없다.
        - 그냥 함수가 리턴되는 것 뿐이다.
        - 그 리턴을 추후 실행하면 함수의 실행이 된다.
    - 함수는 객체이므로 당연하게도 기본(내장) 프로퍼티의 사용이 가능하다.
        - ECMA5 스크립트 명세서에는 모든 함수가 `length`와 `prototype` 프로퍼티를 가져야 한다고 기술하고 있다.
            - `length`
                - 함수가 정상적으로 실행될 때 기대되는 인자의 개수를 나타내는 프로퍼티 (결과가 아니라 인자의 개수이다.)
            - `prototype` (추후 자세히 다룰 예정)
                - `__proto__`와는 다르다.    
        - 자바스크립트의 함수도 당연히 이를 가지고 있다.
        - `name`
            - 함수의 이름을 나타내는 프로퍼티
            - 익명함수의 경우 빈 문자열을 갖는다.
        - `caller`
            - 자신을 호출한 함수를 나타내는 프로퍼티
        - `arguments`
            - 함수 호출시 전달된 인자값들을 나타내는 프로퍼티
        - `__proto__` (추후 자세히 다룰 예정)
            - 자신의 프로토타입을 가리키는 프로퍼티
            
### 5. What is `this`?

- 개요
    - this.. 이 친구는 과연 무엇인가
    - 무엇이길래 한참 전에 봤는데 아직도 잘 모르겠는건가
    - 이제는 더이상 미루지 말고 알아봐야겠다.
  
- this는 무엇을 가리키는가?
    - this는 자신(메서드)을 호출한 객체에 바인딩된다.
    - 다만, 가장 바깥단 (js파일 가장 윗 블럭)에서 쓰이는 함수(메서드)에서는 무얼 가리킬까 의문이 들 수 있다.
    - 그때 호출된 함수의 this는 항상 전역객체로 바인딩이 되며, 브라우저에서는 `window`객체, Node.js에서는 `global`객체를 가리킨다.
    - 때문에 가장 바깥단에 정의된 어떤 함수 `something()`은 `window.something()`으로도 아무런 문제없이 호출이 가능하다.
    - 자바스크립트의 모든 전역 변수는 모두 전역 객체의 프로퍼티들이다.
    
- 생성자 함수에서의 this
    - new 키워드를 통해 생성자(함수)를 호출하면
    - 첫째, 빈객체가 생기고 this는 이 객체를 가리킨다.
    - 둘째, 그 this를 통해 빈객체에 해당 생성자(함수)의 프로퍼티들을 생성한다.
    - 셋째, 리턴 된다. (객체 생성 완료)
    
- `객체 리터럴` vs `new {Something}`
    - 객체 리터럴은 `__proto__`가 Object
    - `new {Something}`은 `__proto__`가 Something
